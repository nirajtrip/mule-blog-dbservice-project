package com.appnovation.rest.dao;
...
import com.appnovation.rest.entity.Employees;


public class EmployeesDao {

	public static final Logger LOGGER = LoggerFactory.getLogger(EmployeesDao.class);
	...
	public List<Employees> findAll() {
		return (List<Employees>) getEntityManager().
				createNamedQuery("Employees.findAll").
				getResultList();
	}	
	...
	public Employees updateEmployees(Employees employee) {
		...
		try {
			...
			LOGGER.error("update operation failed with error: " + ex.getMessage());
		}
		return null;
	}
	...
	public boolean deleteByEmployeeId(String id) {
		...
		try {
			...
			LOGGER.warn("delete called, but no match found for EmployeeId=" + id);
		} catch (Exception ex) {
			LOGGER.error("delete operation failed with error: " + ex.getMessage());
		}
		return false;
	}	
}
